...
import static org.apache.dubbo.rpc.Constants.NICK_CLIENT_HOST_KEY;
...
public class ReferenceConfig<T> extends AbstractReferenceConfig {
    ...
    public void checkAndUpdateSubConfigs() {
        ...
        // 从配置中心刷新数据
        this.refresh();
        // 如果泛化调用 ==null 并且 消费者！=null
        if (getGeneric() == null && getConsumer() != null) {
            // 这里设置的应该是false  （Boolean）
        ...
        // 这一步本来也是init中的操作，被提取到这里了用于从系统属性或配置文件中
        // 加载与接口名相对应的配置，并将解析结果赋值给url 字段
        // 从外部文件解析consumer ur
        resolveFile();
        // 检测 Application 合法性
        checkApplication();
    ...
    public synchronized T get() {
        //这个是官网没有的，把init初始化的（三大步）全放在这里做了
        // 检查更新必要的配置
        ...
        // ref 是否为空，为空则通过 init 方法创建
        if (ref == null) {
            // 该方法主要用于处理配置，以及调用 createProxy 生成代理类
            init();
        }
        return ref;
    }
    ...
    private void init() {
        ...
        /*ConsumerConfig 实例是否存在，如不存在则创建一个新的实例，然后通过
        系统变量或 dubbo.properties 配置文件填充 ConsumerConfig 的字段。
        接着是检测泛化配置，并根据配置设置 interfaceClass 的值*/
        // ------------------------分-------割--------线(1)-----------------------------
        //通过系统变量或 dubbo.properties 配置文件填充 ReferenceConfig 的url字段
        //------------------------分-------割--------线(2)-----------------------------
        // 检测本地存根配置合法性
        ...
        //将metrics、application 、module、ConsumerConfig、ReferenceConfig 等对象的信息添加到map 中
        ...
        //  上--> 处理 MethodConfig 实例。该实例包含了事件通知配置，比如 onreturn、
        // onthrow、oninvoke 等
        // ------------------------分-------割--------线(5)-----------------------------
        //获取服务消费者的IP地址
        ...
        logger.info("原本要注册的地址："+hostToRegistry);
        String nicClientHost = map.get(NICK_CLIENT_HOST_KEY);
        if (!StringUtils.isBlank(nicClientHost)){
            map.put(NICK_CLIENT_HOST_KEY,hostToRegistry);
            hostToRegistry=nicClientHost;
        }
        logger.info("如果标签存在要注册的地址："+hostToRegistry);
        map.put(REGISTER_IP_KEY, hostToRegistry);
        //创建代理类
        ref = createProxy(map);
        // serviceKet===  group/path:version
        String serviceKey = URL.buildKey(interfaceName, group, version);
        // 将 ConsumerModel 存入到 ApplicationModel 中
                                                    // 构建ConsumerModel（serviceKey 和 参数集合map ）
    ...
    private T createProxy(Map<String, String> map) {
        /**
         * 1.如果指定了injvm，则使用它
         * 2.然后如果指定了网址，则假设它是远程呼叫
         * 3.否则，检查范围参数
         * 4.如果未指定范围但在同一JVM中提供目标服务，则更喜欢进行本地调用，这是默认行为
         */
        if (shouldJvmRefer(map)) {
            // 生成本地引用URL   协议为inJVM、本地协议 、本地主机、端口、接口名、map 参数集合
            URL url = new URL(LOCAL_PROTOCOL, LOCALHOST_VALUE, 0, interfaceClass.getName()).addParameters(map);
            // 调用refer 方法构建InJvmInvoder 实例
                ...
                if (!LOCAL_PROTOCOL.equalsIgnoreCase(getProtocol())){
                    ...
                    if (CollectionUtils.isNotEmpty(us)) {
                        for (URL u : us) {
                            ...
                            String refer = StringUtils.toQueryString(map);
                            u = u.addParameterAndEncoded(REFER_KEY, refer);
                            urls.add(u);
                        }
                    }
                    // 未配置注册中心，抛出异常
